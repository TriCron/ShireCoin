...
# TestP2PConn: A peer we use to send messages to shirecoind, and store responses.
...
class CompactBlocksTest(ShirecoinTestFramework):
    ...
    # This test actually causes shirecoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that shirecoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for shirecoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # shirecoind will not send blocktxn responses for blocks whose height is
