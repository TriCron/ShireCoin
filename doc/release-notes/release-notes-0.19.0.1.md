Shirecoin Core version 0.19.0.1 is now available from:

  <https://www.shirecoin.org/bin/shirecoin-core-0.19.0.1/>
...
installer (on Windows) or just copy over `/Applications/Shirecoin-Qt` (on Mac)
or `shirecoind`/`shirecoin-qt` (on Linux).

Upgrading directly from a version of Shirecoin Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of Shirecoin Core are generally supported.

Compatibility
==============

Shirecoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Shirecoin Core on unsupported systems.
...
to open the Shirecoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Shirecoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/tricron/shirecoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Shirecoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Shirecoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `shirecoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Shirecoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Shirecoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Shirecoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Shirecoin Core,
  ...
  This prevents Shirecoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Shirecoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Shirecoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `shirecoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Shirecoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Shirecoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Shirecoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Shirecoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  SHIRE), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in SHIRE per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Shirecoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Shirecoin AEAD (jonasschnelli)
...
- #15639 shirecoin-wallet tool: Drop libshirecoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as SHIRE/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in shirecoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "shirecoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of shirecoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from shirecoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_shirecoin to auto generated project files (sipsorcery)
...
- #15773 Add ShirecoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_shirecoin output (practicalswift)
...
- #15896 feature_filelock, interface_shirecoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Shirecoin Core (luke-jr)
...
- #16277 Suppress output in test_shirecoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as SHIRE/kB (jonatack)
...
- #16489 log: harmonize shirecoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (shirecoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example shirecoin.conf (fanquake)
...
- #16080 build/doc: update shirecoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in shirecoin.conf (fanquake)
...
- #16621 add default shirecoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the shirecoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, shirecoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/shirecoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/shirecoin/shirecoin/).
