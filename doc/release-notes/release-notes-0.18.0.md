Shirecoin Core version 0.18.0 is now available from:

  <https://www.shirecoin.org/bin/shirecoin-core-0.18.0/>
...
`/Applications/Shirecoin-Qt` (on Mac) or `shirecoind`/`shirecoin-qt` (on
Linux).
...
Shirecoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Shirecoin Core on unsupported systems.

Shirecoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Shirecoin Core does not yet change appearance when
...
`contrib/init/shirecoind.service`, it has been changed to use
`/var/lib/shirecoind` as the data directory instead of
`~shirecoin/.shirecoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/shirecoind` will
exist has enough space (check using `df -h /var/lib/shirecoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/tricron/shirecoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:7332:7332` (this is an extra `:7332` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://www.shirecoin.org/en/contact/) the Shirecoin Core
...
The systemd init file (`contrib/init/shirecoind.service`) has been changed
to use `/var/lib/shirecoind` as the data directory instead of
`~shirecoin/.shirecoin`. This change makes Shirecoin Core more consistent with
...
`datadir` option in `/etc/shirecoin/shirecoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/shirecoin/shirecoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/tricron/shirecoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/tricron/shirecoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/tricron/shirecoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/tricron/shirecoin/blob/master/doc/shirecoin-conf.md)
  about the `shirecoin.conf` file describes how to use it to configure
  Shirecoin Core.

- A new document introduces Shirecoin Core's BIP174 [Partially-Signed
  Shirecoin Transactions
  (PSBT)](https://github.com/tricron/shirecoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/tricron/shirecoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Shirecoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Shirecoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/tricron/shirecoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/tricron/shirecoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Shirecoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Shirecoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/tricron/shirecoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/tricron/shirecoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart shirecoind with the `-deprecatedrpc=generate`
...
- A new `shirecoin-wallet` tool is now distributed alongside Shirecoin
...
This section describes planned changes to Shirecoin Core that may affect
other Shirecoin software and services.

- Since version 0.16.0, Shirecoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Shirecoin Core 0.20 (expected about
  a year after 0.18), Shirecoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Shirecoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Shirecoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Shirecoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Shirecoin Core
...
- A sub-project of Shirecoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Shirecoin Core.  See their [project
  page](https://github.com/shirecoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Shirecoin Core's own implementation, although entropy
  gathered by Shirecoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Shirecoin Core a
...
- On macOS, Shirecoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `shirecoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libshirecoin_server.a` (jnewbery)
...
- #13926 [Tools] shirecoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like shirecoind (MarcoFalke)
...
- #14521 qt, docs: Fix `shirecoin-qt -version` output formatting (hebasto)
...
- #11625 Add ShirecoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libshirecoin_server` (laanwj)
...
- #15154 Configure: shirecoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/shirecoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Shirecoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libshirecoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in shirecoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Shirecoin Core (Christewart)
...
- #12246 Bugfix: Only run shirecoin-tx tests when shirecoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_shirecoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_shirecoin` (MarcoFalke)
...
- #14186 shirecoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_shirecoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update shirecoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/shirecoin-conf.md (hebasto)
...
- #15354 Add missing `shirecoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct shirecoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support shirecoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/shirecoin/).
