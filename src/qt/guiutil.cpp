...
#include <qt/shirecoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Shirecoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ShirecoinAddressEntryValidator(parent));
    widget->setCheckValidator(new ShirecoinAddressCheckValidator(parent));
}

bool parseShirecoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no shirecoin: URI
    if(!uri.isValid() || uri.scheme() != QString("shirecoin"))
        return false;
...
bool parseShirecoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseShirecoinURI(uriInstance, out);
}

QString formatShirecoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("shirecoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openShirecoinConf()
{
    ...
    /* Open shirecoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Shirecoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Shirecoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Shirecoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Shirecoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "shirecoin.desktop";
    return GetAutostartDir() / strprintf("shirecoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a shirecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Shirecoin\n";
        else
            optionFile << strprintf("Name=Shirecoin (%s)\n", chain);
