...
static void SetupShirecoinTxArgs()
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " shirecoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  shirecoin-tx [options] <hex-tx> [commands]  Update hex-encoded shirecoin transaction\n" +
            "or:     shirecoin-tx [options] -create [commands]   Create hex-encoded shirecoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded shirecoin transaction
