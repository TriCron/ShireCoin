...
const char * const BITCOIN_CONF_FILENAME = "shirecoin.conf";
 ...
 * https://github.com/tricron/shirecoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //shirecoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "shirecoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Shirecoin
    // macOS: ~/Library/Application Support/Shirecoin
    // Unix-like: ~/.shirecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Shirecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Shirecoin";
#else
    // Unix-like
    return pathRet / ".shirecoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Shirecoin Core copyright is not removed by accident
    if (copyright_devs.find("Shirecoin Core") == std::string::npos) {
