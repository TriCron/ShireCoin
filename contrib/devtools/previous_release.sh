  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/tricron/shirecoin "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/shirecoind src/shirecoin-cli src/shirecoin-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/shirecoin-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="bin/shirecoin-core-${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
        ...
        tar -zxf "shirecoin-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "shirecoin-${tag:1}"
        rm "shirecoin-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
