...
###   shirecoincore.org and shirecoin.org and compares them.
...
WORKINGDIR="/tmp/shirecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://www.shirecoin.org"
HOST2="https://www.shirecoin.org"
BASEDIR="/bin/"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'shirecoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "shirecoin.org failed to provide signature file, but shirecoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "shirecoin.org and shirecoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Shirecoin Core binary release signing key installed?"
   fi
