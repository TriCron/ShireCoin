...
// shirecoin includes.
#include <..\src\script\shirecoinconsensus.h>
...
int main()
{
    std::cout << "shirecoinconsensus version: " << shirecoinconsensus_version() << std::endl;
    ...
    auto op0Result = shirecoinconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, shirecoinconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
